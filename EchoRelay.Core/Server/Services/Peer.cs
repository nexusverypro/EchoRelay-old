...
namespace EchoRelay.Core.Server.Services
{
    ...
    public class Peer
    {
        ...
        {
            ConsoleLogger.LogMessage(LogType.Debug, "(Peer:{0}) Updating user authentication", Id);
        ...
        {
            ConsoleLogger.LogMessage(LogType.Debug, "(Peer:{0}) Sending {1} message(s). Messages = [{2}]", Id, messages.Count(), string.Join(", ", from x in messages 
        ...
        {
            ConsoleLogger.LogMessage(LogType.Debug, "(Peer:{0}) Sending {1} message(s). Messages = [{2}]", Id, packet.Count(), string.Join(", ", from x in packet
                                                                                                                                                 select x.GetType().Name));
