...
namespace EchoRelay.Core.Server.Services.Config
{
    ...
    public class ConfigService : Service
    {
        ...
        private async Task ProcessConfigRequestv2(Peer sender, ConfigRequestv2 request)
        {
            ...
            if (typeSymbol == null)
            {
                ConsoleLogger.LogMessage(LogType.Error, "(ConfigService) 'typeSymbol' was null! Discarding request.", sender.Id);
            ...
            {
                ConsoleLogger.LogMessage(LogType.Error, "(ConfigService) 'identifierSymbol' was null! Discarding request.", sender.Id);
            ...
            if (configData == null)
            {
                ConsoleLogger.LogMessage(LogType.Error, "(ConfigService) 'configData' was null! Discarding request.", sender.Id);
