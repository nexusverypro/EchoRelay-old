...
namespace EchoRelay.Core.Server.Services.Matching
{
    public class MatchingSession
    {
        ...
        {
            ConsoleLogger.LogMessage(LogType.Warning, "(MatchingSession) New session created [userId = {0}, lobbyId = {1}, channel = {2}, settings = {3}]", 
                userId.ToString(), 
                lobbyId == null ? "None" : lobbyId,
                channel == null ? "None" : channel,
