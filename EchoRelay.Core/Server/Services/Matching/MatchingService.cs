...
namespace EchoRelay.Core.Server.Services.Matching
{
    public class MatchingService : Service
    {
        ...
        private async Task ProcessMatchingSession(Peer sender, Guid session, XPlatformId userId)
        {
            ...
            {
                ConsoleLogger.LogMessage(LogType.Error, "(MatchingService) Unauthorized");
            ...
            if (account == null)
            {
                ConsoleLogger.LogMessage(LogType.Error, "(MatchingService) Failed to obtain profile");
            ...
            if (account.Banned)
            {
                ConsoleLogger.LogMessage(LogType.Error, "(MatchingService) User is banned");
            ...
            {
                ConsoleLogger.LogMessage(LogType.Error, "(MatchingService) Cannot process session request, no matching session exists");
            ...
            {
                ConsoleLogger.LogMessage(LogType.Error, "(MatchingService) User is not a moderator");
            ...
            if (matchingSession.LobbyId != null)
            {
                ...
                {
                    ConsoleLogger.LogMessage(LogType.Error, "(MatchingService) Could not find requested lobby id");
            ...
            ConsoleLogger.LogMessage(LogType.Info, "(MatchingService) Found {0} game server(s)", gameServers.Count());
        ...
        private async Task ProcessPingResponse(Peer sender, LobbyPingResponse request)
        {
            ...
            {
                ConsoleLogger.LogMessage(LogType.Info, "(MatchingService) Could not obtain registered game server to serve request");
        ...
        private async Task ProcessPlayerSessionsRequestv5(Peer sender, LobbyPlayerSessionsRequestv5 request)
        {
            ...
            if (matchingSession == null)
            {
                ConsoleLogger.LogMessage(LogType.Error, "(MatchingService) Player sessions requested, but no matching session exists");
            ...
            {
                ConsoleLogger.LogMessage(LogType.Error, "(MatchingService) Player sessions requested, but no matched game server exists");
        ...
        internal async Task SendLobbySessionFailure(Peer peer, LobbySessionFailureErrorCode errorCode, string errorMessage)
        {
            ...
            if (matchingSession == null)
            {
                ConsoleLogger.LogMessage(LogType.Error, "(MatchingService.SendLobbySessionFailure) Matching session is null.");
                return;
            }
