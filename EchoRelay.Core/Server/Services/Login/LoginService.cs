...
namespace EchoRelay.Core.Server.Services.Login
{
    ...
    public class LoginService : Service
    {
        ...
        private async Task ProcessLoginRequest(Peer sender, LoginRequest request)
        {
            ...
            // This should be ran first to avoid any accounts being created
            if (displayNameOverride.ToLower().Contains("account"))
            {
                await sender.Send(new LoginFailure(request.UserId, HttpStatusCode.BadRequest, "Cannot make an account that has 'account' in the username."));
                return;
            }
            ...
            {
                ConsoleLogger.LogMessage(LogType.Error, "(LoginService) The UserId {0} is not valid.", request.UserId.ToString());
            ...
            {
                ConsoleLogger.LogMessage(LogType.Error, "(LoginService) Authentication failed. The account info id is incorrect.");
            ...
            {
                ConsoleLogger.LogMessage(LogType.Error, "(LoginService) Authentication failed.");
