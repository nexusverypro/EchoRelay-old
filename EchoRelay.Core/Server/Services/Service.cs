...
namespace EchoRelay.Core.Server.Services
{
    ...
    public abstract class Service
    {
                ...
                while (webSocket.State == WebSocketState.Open)
                {
                    ...
                    switch (messageType)
                    {
                        ...
                        case WebSocketMessageType.Close:
                            ...
                            return;
                        default:
                            ConsoleLogger.LogMessage(LogType.Error, "(Service:{0}) Received an unexpected websocket message type. Peer Id = {1}", GetType().Name, peer.Id);
        ...
        {
            ConsoleLogger.LogMessage(LogType.Trace, "(Service:{0}) Packet sent. Packet Id = {1}, Peer Id = {2}", GetType().Name, packet.GetHashCode(), peer.Id);
        ...
        {
            ConsoleLogger.LogMessage(LogType.Debug, "(Service:{0}) Peer authenticated! Peer Id = {1}", GetType().Name, peer.Id);
        ...
        {
            ConsoleLogger.LogMessage(LogType.Trace, "(Service:{0}) Packet received. Packet Id = {1}, Peer Id = {2}", GetType().Name, packet.GetHashCode(), peer.Id);
