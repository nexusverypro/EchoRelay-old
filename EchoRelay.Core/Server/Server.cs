...
namespace EchoRelay.Core.Server
{
    ...
    public class Server
    {
        ...
        {
            ConsoleLogger.LogMessage(LogType.Info, "Starting EchoRelay server");
            // If we are running already, throw an exception.
            if (Running)
            {
                ConsoleLogger.LogMessage(LogType.Warning, "Server cannot be started if it is already in a running state.");
            ...
            // Start the listener
            listener.Start();
            ConsoleLogger.LogMessage(LogType.Info, "Server started on port " + Settings.Port);
            ...
            try
            {
                while (!_cancellationTokenSource.IsCancellationRequested)
                {
                    ...
                    if (!listenerContext.Request.IsWebSocketRequest)
                    {
                        ...
                        ConsoleLogger.LogMessage(LogType.Warning, "User tried to connect without WebSocket protocol! Discarding this request.");
                    ...
                    {
                        // TODO: Log the interaction.
                        ConsoleLogger.LogMessage(LogType.Warning, "User tried to connect without being authorized! Discarding this request.");
                    ...
                    try
                    {
                        ...
                        ConsoleLogger.LogMessage(LogType.Info, "User connected");
                    }
                    catch (Exception e)
                    {
                        ...
                        ConsoleLogger.LogMessage(LogType.Error, "User failed to connect: " + e.ToString());
                        continue;
                    }
                    ...
                    if (listenerContext.Request.Url == null || !_serviceMap.TryGetValue(listenerContext.Request.Url.LocalPath.ToLower() ?? "", out var service))
                    {
                        ...
                        // TODO: Log the interaction.
                        string friendlyServiceName = listenerContext.Request.Url!.LocalPath.ToLower().TrimStart('/');
                        ConsoleLogger.LogMessage(LogType.Warning, "User tried to connect to the '{0}' service, but it does not exist.", friendlyServiceName);
                        continue;
                    }
            ...
            {
                ConsoleLogger.LogMessage(LogType.Error, "Server timeout: " + tEx.ToString());
            }
            catch (TaskCanceledException tcEx)
            {
                ConsoleLogger.LogMessage(LogType.Error, "Server task cancelled: " + tcEx.ToString());
            }

            // Ensure our listener is closed
            listener.Close();
            ConsoleLogger.LogMessage(LogType.Warning, "Closed server");
